/**
 * SEARCH INDEX BUILDER — WEBFLOW × AIRTABLE JOIN
 * ────────────────────────────────────────────────────────────────────────────
 ...
 *   Join key: Airtable `Webflow Item ID` → Webflow item id.
 *
 ...
 *   See PROHIBITED_PATTERNS. Dropped docs are logged so the exclusion is auditable.
 *
 * COST DISCIPLINE
 ...
 *
 * INVOKE
 *   Scheduled 6-hourly via netlify.toml. For manual runs / validation:
 *     GET /.netlify/functions/search-index-build?key=<SEARCH_INDEX_KEY>
 *     GET /.netlify/functions/search-index-build?key=<...>&dry=1   → build, report,
 *         do NOT write to Blobs. Returns counts + a sample doc. Use this first.
 *
 * Env: WEBFLOW_API_TOKEN, AIRTABLE_API_KEY (or AIRTABLE_TOKEN), SEARCH_INDEX_KEY,
...
async function build() {
  const stats = {
    prohibitedDropped: [], unjoined: 0, excluded: {},
    variantsFolded: 0, faqsFolded: 0, faqsOrphaned: 0,
  };
  const bump = (k) => { stats.excluded[k] = (stats.excluded[k] || 0) + 1; };

  /* 1 ── Webflow: what actually has a live URL. */
  const [wfProducts, wfBrands, wfPrimary, wfParent, wfSub] = [
    await wfCollectionItems(WF.products,   'products'),
    await wfCollectionItems(WF.brands,     'brands'),
    await wfCollectionItems(WF.catPrimary, 'primary-categories'),
    await wfCollectionItems(WF.catParent,  'parent-categories'),
    await wfCollectionItems(WF.catSub,     'subcategories'),
  ];

  /* 2 ── Airtable: ranking signals + the variant/FAQ graph. */
  const atProducts = await atFetchAll(AT_PRODUCTS, AT_PRODUCT_FIELDS, 'products');
  await sleep(THROTTLE_MS);
  const atVariants = await atFetchAll(AT_VARIANTS, AT_VARIANT_FIELDS, 'variants');
  await sleep(THROTTLE_MS);
  const atFaqs = await atFetchAll(AT_FAQS, AT_FAQ_FIELDS, 'faqs');

  const variantById = new Map(atVariants.map((v) => [v.id, v.f]));
  const faqById     = new Map(atFaqs.map((q) => [q.id, q.f]));

  // Airtable products keyed by Webflow item id — the join key.
  ...
  for (const item of wfProducts) {
    ...
    if (truthy(a.Discontinued))  { bump('product:discontinued');  continue; }
    ...
    for (const qid of faqIds) {
      ...
      faqText.push(truncate(stripHtml(q.Answer), 200));
    }
    ...
    // Compliance guard — check everything searchable, not just the name.
    const haystack = [name, brand, folded, faqs, desc, cats.join(' ')].join(' ');
    if (isProhibited(haystack)) {
...
exports.handler = async (event) => {
  ...
  const isSchedule = !!(event && event.body && !event.httpMethod);

  // Manual invocations need the key; the scheduler does not present one.
  if (!isSchedule) {
  ...
  try {
    ...
    const report = { ok: true, dry, bytes, ms: Date.now() - started, ...stats };
    console.log('[search-index]', JSON.stringify(report));
    return {
      statusCode: 200,
    ...
    console.error('[search-index] FAILED', err && err.message ? err.message : err);
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' },
      body: JSON.stringify({ ok: false, error: String((err && err.message) || err), ms: Date.now() - started }),
    };
  }
};
