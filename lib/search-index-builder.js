 ...
 *
 *   NEGATIONS ARE NOT MENTIONS. Several product descriptions read "Does not
 *   contain 7OH or MGM" — a statement of compliance, not a compliance problem.
 *   Matching those flagged 8 clean products and dropped the /simple-brands page
 *   outright. A review list that is 100% false alarms does not get read, which
 *   defeats the point of having one, so mentions preceded by a negation cue
 *   ("does not contain", "free of", "no", "without", …) are ignored.
 *
 *   Every document type uses the same rule: drop on identity, flag on unnegated
 *   discussion. Pages previously matched on their SEO description, which is why
 *   /simple-brands disappeared while equivalent products only got flagged.
 *
...
/* ⚠️ THIS MODULE MUST HAVE NO IMPORTS. It is CommonJS (so the offline tests can
 * require it) but gets bundled into ESM v2 functions, and esbuild turns a
 * top-level require() of anything external into a shim that THROWS:
 *   Error: Dynamic require of "https" is not supported
 * So: global fetch instead of node:https, and the Netlify Blobs write lives in the
 * .mts wrappers where a static import works. Keep it that way — adding a require
 * here breaks the deploy, and it fails at import time, before any handler runs. */
...
/* Minimal GET helper over global fetch (Node 18+, and present on the Netlify
 * runtime). Returns the same shape the rest of this module expects. */
async function httpsReq(url, opts) {
...
function stripHtml(v) {
  return clean(String(v == null ? '' : v)
    .replace(/<[^>]*>/g, ' ')
    .replace(/&[a-z]+;|&#\d+;/gi, ' ')
    // Some descriptions contain literal markdown ("**_Does not contain 7OH._**").
    // Runs of 2+ emphasis characters are always markup; single ones are left be,
    // since a lone underscore can legitimately appear in product text.
    .replace(/[*_]{2,}/g, ''));
}
...
/* Negation cues that turn a mention into a disclaimer rather than an ingredient. */
const NEGATION_RE = /\b(?:does\s*n[o']?t|do\s*n[o']?t|doesn|dont|no|non|never|free\s+of|without|contains\s+no|zero|excludes?)\b/i;
const NEGATION_WINDOW = 45;

/* True only if the text mentions a prohibited term WITHOUT a nearby negation.
 * "Does not contain 7OH" is a compliance statement, not a compliance risk — and
 * treating it as one buried the review list in false positives. */
function mentionsProhibited(text) {
  const str = String(text || '');
  for (const re of PROHIBITED_PATTERNS) {
    const rx = new RegExp(re.source, re.flags.includes('g') ? re.flags : re.flags + 'g');
    let m;
    while ((m = rx.exec(str)) !== null) {
      /* Normalise punctuation before testing. Descriptions carry raw markdown, and
       * "**_Does not contain 7OH_**" defeated a -anchored cue because underscore
       * is itself a word character — so the negation went undetected. */
      const lookback = str
        .slice(Math.max(0, m.index - NEGATION_WINDOW), m.index)
        .replace(/[^A-Za-z0-9\s]+/g, ' ');
      if (!NEGATION_RE.test(lookback)) return true;   // an unqualified mention
      if (rx.lastIndex === m.index) rx.lastIndex++;   // guard against zero-width
    }
  }
  return false;
}
...
async function build() {
  /* Reset per build. These are module-scoped, so on a warm container they
   * accumulate across invocations and misreport — a doubled count was the clue
   * that a report had come from a previous, stale deploy. */
  airtableCalls = 0;
  webflowCalls = 0;

  const stats = {
    prohibitedDropped: [], flaggedForReview: [], unjoined: 0, excluded: {},
    variantsFolded: 0, faqsFolded: 0, faqsOrphaned: 0, categoriesWithBrands: 0, categoryBrandsPropagated: 0,
    sourceErrors: [],
  };
  ...
  const wfBlog    = await optional('webflow:blog',      () => wfCollectionItems(WF.blog,      'blog-posts'), []);
  ...
  /* category name -> Set of brand names, filled in by the product loop below. */
  ...
  for (const item of wfProducts) {
    ...
    // Negated mentions ("Does not contain 7OH") are disclaimers, not risks.
    if (mentionsProhibited([faqs, desc].join(' '))) {
    ...
    /* Brand -> category association, accumulated as products are indexed.
     *
     * Searching a brand should surface the categories it sells into. Nothing in a
     * category record names its brands, and nothing in a brand record names its
     * categories — the relationship only exists on the products, so it has to be
     * derived here. Without it, "tre house" returned no categories at all unless a
     * category's own meta description happened to mention the brand.
     *
     * Placed immediately before the push so it counts exactly the products that make
     * it into the index: everything dropped by the guards above has already
     * `continue`d, so a filtered or non-compliant product cannot contribute a brand. */
    if (brand) {
      for (const c of cats) {
        let set = catBrands.get(c);
        if (!set) catBrands.set(c, (set = new Set()));
        set.add(brand);
      }
    }
    ...
    docs.push({
      ...
      st: inStock,
      /* Yotpo aggregate, read from WEBFLOW rather than Airtable or the Yotpo API.
       * The values are already synced into the CMS, so no extra credentials and no
       * extra calls. Note `starrating` ("Product Rating") is a dead legacy field —
       * empty on all 12 best-sellers sampled — so do not switch to it. */
      r: num(wf['yotpo-rating']),
      rc: num(wf['yotpo-review-count']),
  ...
  for (const item of wfBrands) {
    ...
    if (isProhibited(name)) { stats.prohibitedDropped.push('brand:' + name); continue; }
    if (mentionsProhibited(desc)) stats.flaggedForReview.push('brand:' + name);
    docs.push({
      t: 'brand',
      n: name,
  ...
  /* 4b ── BLOG POSTS. 307 published articles that search could not reach at all: the
   * current box filters the product list, so every article was invisible to it, and the
   * index had no `blog` documents either.
   *
   * INDEXES THE SUMMARY, NOT THE BODY. `post-body` is rich text running to thousands of
   * words per post; folding 307 of those in would multiply the index size for text that
   * is mostly prose a searcher never types. Title plus summary is what matches a query
   * like "how to store gummies" without turning a 93KB payload into a megabyte. If body
   * search is ever wanted, it needs a server-side index, not this one.
   *
   * Drafts and archived posts are already excluded by wfCollectionItems, so nothing here
   * can link to an unpublished article. */
  for (const item of wfBlog) {
    const wf = item.f;
    const name = clean(wf.name);
    const slug = clean(wf.slug);
    if (!name || !slug) { bump('blog:no name/slug'); continue; }
    const desc = truncate(stripHtml(wf['post-summary'] || wf['meta-description'] || ''), 200);
    /* Same rule as every other document type: drop on identity, flag on discussion.
     * Educational articles legitimately discuss 7-OH, so a mention in the summary is
     * reported for a human rather than silently deleted — see the header note. */
    if (isProhibited(name)) { stats.prohibitedDropped.push('blog:' + name); continue; }
    if (mentionsProhibited(desc)) stats.flaggedForReview.push('blog:' + name);
    docs.push({
      t: 'blog',
      n: name,
      u: WF_PATH.blog + slug,
      img: (wf['thumbnail-image'] && clean(wf['thumbnail-image'].url))
        || (wf['main-image'] && clean(wf['main-image'].url)) || undefined,
      d: desc || undefined,
    });
  }

  /* Roll each parent category's brand list UP into the primary category above it.
   *
   * Products only carry PARENT categories, so the product loop above fills that one
   * level. A brand sold in the parent "Delta 8" is by definition sold within whatever
   * primary category Delta 8 belongs to, so this direction adds real information: it is
   * what makes a brand search return "THC" and "Shrooms".
   *
   * NAMING TRAP: the field SLUG here is `sub-category`, but its display name is "Primary
   * Categories" and it references the PRIMARY collection. The slug is simply wrong.
   * Reading it as a subcategory would file every brand under the wrong level. Populated
   * on 55 of 59 parents.
   *
   * DOWNWARD PROPAGATION IS DELIBERATELY NOT DONE, though the data is right there and it
   * would have filled the 11 empty subcategories, "Vape Pens" among them — and "vape
   * pens" is the highest-volume search term on the site. It would also have been wrong.
   * Parent categories are CANNABINOID TYPES (Delta 8, THCa, Live Resin) and
   * subcategories are FORM FACTORS (Vape Pens, Gummies & Edibles, Dabs, Tinctures).
   * Every cannabinoid parent lists all 7 forms beneath it, so inheriting brands downward
   * would claim a brand selling only Delta 8 gummies also sells Delta 8 vape pens. The
   * two axes are orthogonal, and nothing available links a product to its form: the
   * subcategory `products` field is empty on all 11 records, and products carry no
   * subcategory reference. An empty brand list is honest; a fabricated one is not. */
  const primaryNameById = new Map();
  for (const item of wfPrimary) {
    const n = clean(item.f && item.f.name);
    if (n) primaryNameById.set(item.id, n);
  }
  for (const item of wfParent) {
    const wf = item.f || {};
    const brands = catBrands.get(clean(wf.name));
    if (!brands || !brands.size) continue;
    const target = primaryNameById.get(wf['sub-category']);
    if (!target || target === clean(wf.name)) continue;
    let set = catBrands.get(target);
    if (!set) catBrands.set(target, (set = new Set()));
    const before = set.size;
    for (const b of brands) set.add(b);
    if (set.size > before) stats.categoryBrandsPropagated++;
  }
  ...
  for (const [key, items] of [['catPrimary', wfPrimary], ['catParent', wfParent], ['catSub', wfSub]]) {
    for (const item of items) {
      ...
      if (isProhibited(name)) { stats.prohibitedDropped.push(key + ':' + name); continue; }
      if (mentionsProhibited(desc)) stats.flaggedForReview.push(key + ':' + name);
      /* Joined on name, which matched 56/56 of the distinct category values carried by
       * products in the 2026-08-02 index. Sorted so the index stays byte-stable between
       * builds and the ETag does not churn. Sub and primary levels are filled in by the
       * propagation step above, not by products directly. */
      const catBrandList = catBrands.get(name);
      if (catBrandList && catBrandList.size) stats.categoriesWithBrands++;
      docs.push({
        t: 'category',
        n: name,
        u: WF_PATH[key] + slug,
        d: desc || undefined,
        br: catBrandList && catBrandList.size ? [...catBrandList].sort() : undefined,
      });
    }
  }
  ...
  for (const pg of pages) {
    ...
    /* Identity is the title and the URL. The SEO description is body text — it was
     * included here originally, which dropped /simple-brands for carrying a
     * "does not contain 7OH" disclaimer. */
    if (isProhibited([name, path].join(' '))) { stats.prohibitedDropped.push('page:' + path); continue; }
    if (mentionsProhibited(desc)) stats.flaggedForReview.push('page:' + path);
  ...
  return {
    ...
    stats: {
      /* Fingerprint of the deployed code, so a report from a stale deploy is
       * distinguishable from a fix that did not work — that ambiguity cost a
       * debugging round trip.
       *
       * Hashing the bundle rather than reading COMMIT_REF/BRANCH: those are
       * build-time variables and are NOT scoped to the function runtime, so they
       * came back "unknown". __filename is the esbuild bundle, whose hash changes
       * whenever any bundled source changes — no Netlify metadata required. */
      /* Set by the .mts wrapper, which can statically import node:crypto and
       * node:fs. Computing it here would need a require() — see the warning at the
       * top of this file. */
      codeVersion: null,
      docs: docs.length,
      byType: docs.reduce((m, d) => (m[d.t] = (m[d.t] || 0) + 1, m), {}),
      webflowLive: {
        products: wfProducts.length, brands: wfBrands.length, blogPosts: wfBlog.length,
      ...
      categoriesWithBrands: stats.categoriesWithBrands,
      categoryBrandsPropagated: stats.categoryBrandsPropagated,
...
/* NOTE: writing to Netlify Blobs deliberately does NOT live here. getStore() needs
 * `require('@netlify/blobs')`, which becomes a throwing shim once this CommonJS
 * module is bundled into an ESM function. The .mts wrappers import it statically
 * and pass the built index to it. BLOB_STORE / BLOB_KEY are exported so both
 * wrappers write to the same place. */

/* Length-first comparison so a wrong key does not leak its length via early exit.
 * Lives here rather than in the .mts wrapper so the offline tests can cover it —
 * a CommonJS test cannot require an ESM wrapper. */
function keyMatches(got, want) {
  const g = String(got == null ? '' : got);
  const w = String(want == null ? '' : want);
  if (!w) return false;
  return g.length === w.length && g === w;
}

module.exports = { build, keyMatches, BLOB_STORE, BLOB_KEY, INDEX_VERSION };
