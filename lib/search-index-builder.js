...
/* Hash of the deployed bundle. Computed once and cached; falls back to 'unknown'
 * rather than failing a build over a diagnostic. */
...
function codeFingerprint() {
  ...
  try {
    ...
    const fs = require('fs');
    _fingerprint = crypto.createHash('sha1').update(fs.readFileSync(__filename)).digest('hex').slice(0, 8);
  } catch (_) {
 ...
 * Lives here rather than in the wrapper so it stays covered by the offline tests —
 * the wrappers are .mts and cannot be required from a CommonJS test. */
...
module.exports = { build, writeIndex, keyMatches, BLOB_STORE, BLOB_KEY, INDEX_VERSION };
